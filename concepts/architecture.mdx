...
The `src/context/` directory houses global React context providers that wrap the entire application inside `src/main.tsx`:

| Provider | Purpose |
|---|---|
| `ThemeProvider` | Light / dark mode, synced to a cookie |
| `FontProvider` | Active font family preference |
| `DirectionProvider` | LTR / RTL text direction |
| `LayoutProvider` | Fixed vs. scrollable content layout |
| `SearchProvider` | Global command-palette search state |
...
- **`useMobile`** — Breakpoint hook that returns `true` when the viewport is in the mobile range.
