...
```ts
// Typical usage inside a feature route component
const { search, navigate } = Route.useRouteContext()
...
<CardGroup cols={2}>
  ...
  <Card title="defaultPreloadStaleTime: 0" icon="clock-rotate-left">
    Prefetched data is never considered stale, so navigating back to a prefetched route does not trigger an unnecessary re-fetch.
  </Card>
</CardGroup>
