...
description: "Full reference of all Corpointa REST API endpoints — authentication, dashboard stats, materials, stock, movements, contacts, users, and audit logs."
---
...
<ParamField body="detalles" type="array" required>
  Array of line items. Each object must include `fk_id_material` (number) and `cantidad` (number).
</ParamField>
...
<ParamField body="fk_id_gerente_finanzas" type="number">
  Foreign key of the finance manager approving the dispatch. Optional.
</ParamField>

<ParamField body="fk_id_presidente" type="number">
  Foreign key of the president/director approving the dispatch. Optional.
</ParamField>
...
System user accounts that can log into Corpointa. The actual backend routes use `/users` (not `/usuarios`).
