...
sidebarTitle: "Stock"
description: "Existencias tracks current stock per material with configurable minimum and maximum thresholds. Low-stock items are flagged on the dashboard and in the stock table."
---
...
Both collections are mapped to the unified `Acta` shape and returned as a single sorted array. If the salidas endpoint is unavailable, it gracefully returns an empty array rather than failing.
