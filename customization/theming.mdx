---
title: "Customize Corpointa Themes, Colors, and Dark Mode"
sidebarTitle: "Theming"
description: "Corpointa supports light, dark, and system themes using CSS variables with TailwindCSS v4. Users can switch themes in Settings, and the preference persists via cookie."
---
...
Corpointa ships with three theme modes: **light**, **dark**, and **system**.
...
- **System** — follows the operating system's `prefers-color-scheme` media query and switches automatically if the OS preference changes
...
Dark mode is toggled by adding or removing the `dark` class on the `<html>` element. The `ThemeProvider` context handles this:
...
Corpointa supports right-to-left layouts for Arabic, Hebrew, and other RTL languages. The text direction is managed by `DirectionProvider` and toggled from the **Config Drawer** under the **Direction** section, which offers LTR and RTL options.
...
The direction preference is stored in the `dir` cookie (1-year expiry) and restored on load. Radix UI components (used by all shadcn/ui primitives) receive the direction value through `@radix-ui/react-direction`'s `DirectionProvider`, so popover positions, dropdown alignments, and sidebar chevron directions all flip automatically with zero extra configuration.
